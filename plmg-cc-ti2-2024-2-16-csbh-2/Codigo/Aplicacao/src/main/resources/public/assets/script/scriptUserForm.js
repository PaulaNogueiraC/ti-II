...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (usuarioLogado.fotos.length == 0) {
  ...
  document.querySelector('input[type="button"]').addEventListener('click', async function () {
    ...
    if (!isEmailValid(email)) {
      document.getElementById("erro").innerHTML = `<p>Por favor, digite um e-mail válido.</p>`;
    setTimeout(() => {
      erro.innerHTML = '';
  }, 1200);
      return;
    ...
    let passwordStorage = usuarioLogado.senha;
    const response = await fetch('http://localhost:6789/autenticador', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
          senhaCriptografada: usuarioLogado.senha,
    ...
    let usuario = { nome: nome, email : email, senha: password, fotos: arrayFinal };
    fetch(`http://localhost:6789/voluntario/update/${usuarioLogado.id}`, {
      method: 'POST',
      headers: {
      ...
      .then(async data => {
        const responseUsuario = await fetch(`http://localhost:6789/voluntario/${usuarioLogado.id}`);
        const usuarioBD = await responseUsuario.json();
        document.getElementById("suces").innerHTML = `<p>Dados atualizados com sucesso.</p>`;
        usuario = { id: usuarioLogado.id, nome, email, senha: usuarioBD.senha, fotos: arrayFinal };
        localStorage.setItem('usuarioLogado', JSON.stringify(usuario)); // Atualiza o objeto no localStorage
        console.log(usuario);
        setTimeout(() => {
