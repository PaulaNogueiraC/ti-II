...
const submitForm = formLogin.querySelector('input[type="submit"]');
...
document.getElementById('btnFaceId').addEventListener('click', function() {
    ...
    document.getElementById('photo-container').innerHTML = `<p>Por favor, capture ou faça o upload de 4 fotos para o seu Face ID.</p>
                ...
                document.querySelector('.page').style.marginTop = '520px'
                document.querySelector('.page').style.marginBottom = '380px'
...
submitForm.addEventListener('click', async (event) => {
    ...
    const novoUsuario = { nome, email, senha: password, info, fotos: arrayFinal};

    try {
        const response = await fetch('http://localhost:6789/anfitriao/insert', {
            method: 'POST',
